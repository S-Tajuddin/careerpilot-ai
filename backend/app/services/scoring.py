...
class ScoringService:
    ...
    def _score_skills(self, job: Job, profile: Profile) -> float:
        ...
        # If profile has resume_text but skills are still empty, extract from resume
        if not profile_skills and hasattr(profile, 'resume_text') and profile.resume_text:
            profile_skills = self._extract_skills_from_text(profile.resume_text)
        # Final fallback only if no resume and no skills at all
    ...
    def _get_strengths(self, skill_s, exp_s, salary_s, loc_s, comp_s, remote_s, job, profile) -> list[str]:
        strengths = []
        has_resume = bool(hasattr(profile, 'resume_text') and profile.resume_text)
        source = "resume" if has_resume else "profile"
        if skill_s >= 70: strengths.append(f"Strong skill alignment ({skill_s:.0f}%) — your {source} skills match this role")
        if exp_s >= 80: strengths.append(f"Experience level matches well ({profile.experience_years or 8:.0f} years)")
        min_lpa = (profile.expected_salary_min or 2_000_000) // 100000
        max_lpa = (profile.expected_salary_max or 3_500_000) // 100000
        if salary_s >= 80: strengths.append(f"Salary range aligns with your target (₹{min_lpa}-{max_lpa} LPA)")
