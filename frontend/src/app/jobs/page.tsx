      ...
      {!hasSearched ? (
        ...
        <div className="glass-card p-16 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-md mx-auto mb-4">
            {hasResume
              ? 'Use the search bar above, click a quick search chip, or hit "Find Jobs for Me" to discover AEM/EDS roles matched to your resume.'
              : 'Type a search query above or click one of the quick search chips to discover AEM/EDS roles. Upload your resume for personalized results.'}
          </p>
          <div className="flex justify-center gap-3">
            ...
            <button onClick={() => { if (searchChips.length > 0) handleQuickSearch(searchChips[0].query, searchChips[0].location); }} className="btn-secondary text-sm flex items-center gap-2">
              <HiOutlineBriefcase className="w-4 h-4" /> Try: {searchChips[0]?.label || 'AEM Developer'}
            </button>
          </div>
        </div>
      ) : loading ? (
