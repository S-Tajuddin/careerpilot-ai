...
export const generateCoverLetter = (data: { job_id: number; tone?: string }) =>
  fetchAPI('/api/documents/cover-letter', {
    method: 'POST',
    body: JSON.stringify(data),
  });

export const getCoverLettersForJob = (jobId: number) =>
  fetchAPI(`/api/documents/cover-letter/job/${jobId}`);

export const getCoverLetter = (id: number) =>
  fetchAPI(`/api/documents/cover-letter/${id}`);

export const tailorResume = (jobId: number) =>
  fetchAPI(`/api/documents/tailor-resume?job_id=${jobId}`, { method: 'POST' });

export const getTailoredResume = (jobId: number) =>
  fetchAPI(`/api/documents/tailored-resume/${jobId}`);

export const downloadCoverLetterUrl = (id: number) =>
  `${API_URL}/api/documents/download/cover-letter/${id}`;
